...
class Books extends Controller
{
	...
	public function store()
	{
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['book-register'])) {
			if (
				isset($_POST['book-title']) && isset($_POST['book-isbn']) &&
				...
				if ($this->booksModel->addBook($param, 1, $cantidadBook)) { //hacer asignacion a una variablee , 1 es la cantidad para iniciar en el libro
					echo '<p>guardado con exito<p>';
					redirect('books/create?confir=bien');
				} else {
					echo "FATAL ERROR";
				}
			}
				...
				echo "
						<li class='option'>
							<div class='col-12 p-1'>
								<div class='media'>
									<img class='mr-3' src='" . URL_ROUTE . "media/images/book/$book->book_img' alt='User profile img' style='width: 50px; height:70px;'>
									<div class='media-body'>
										<h5 class='mt-0'>$book->book_title</h5>";
				$authorHasBook = $this->authorHasBookModel->getAutorTipe($book->book_id);
				foreach ($authorHasBook as $authorsHasBook) {
					echo "<p style='font-size:80%'>$authorsHasBook->author_name $authorsHasBook->author_lastname</p>";
				}
				echo "<input type='hidden' name='isbn' value='$book->book_isbn'></div><span class='material-icons'>event_note</span></li>";
			}
		}
	}
	...
	public function update()
	{
		if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['book-update'])) {
			if (
				...
				redirect('books/index?confir=bien');
			}
		}
	}
	...
	public function delete($book_id) //ELIMINAR LIBRO
	{
